...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pokemon_id'])) {
    ...
    // 1. Verificamos que el rescatista tenga permiso (que el pokemon sea de su equipo)
    ...
    if ($poke && $poke->equipo_id == $user->equipo_id) {
        // 2. Actualizamos el estado y añadimos la nota a la bitácora (descripción)
        $fecha = date('d/m/Y');
